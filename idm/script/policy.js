...
var _ = require('lib/lodash'),
    ...
    "policies" : [
        ...
        {   "policyId" : "password-blacklisted",
            "policyExec" : "passwordBlacklisted",
            "policyRequirements" : ["PASSWORD_BLACKLISTED"]
        },
        {   "policyId" : "required",
            "policyExec" : "required",
            "clientValidation": true,
...
policyImpl = (function (){
    ...
    policyFunctions.passwordBlacklisted = function(fullObject, value, params, property) {
        if (value !== undefined){
            var passwords  = java.nio.file.Files.readAllLines( java.nio.file.Paths.get(params.filePath));
            var i;
            for (i = 0; i < passwords.size(); i++) {
                if(value.equalsIgnoreCase(passwords.get(i))){
                    return [ { "policyRequirement" : "PASSWORD_BLACKLISTED" } ];
                }
            }
        }
