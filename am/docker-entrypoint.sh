...
echo "[docker-entrypoint.sh] Command: $command"

export CONFIGURATION_LDAP="${CONFIGURATION_LDAP:-localhost:1389}"
echo "[docker-entrypoint.sh] CONFIGURATION_LDAP = $CONFIGURATION_LDAP"
...
wait_configstore_up() {
    echo "[docker-entrypoint.sh] Waiting for the configuration store to come up...."
    sleep 10
    ...
    do
        ...
        if [ $? = 0 ];
        then
            echo "[docker-entrypoint.sh] Configuration store is up"
            break;
        fi
        sleep 10
        echo -n "."
...
is_configured() {
    ...
    echo "[docker-entrypoint.sh] Testing if the configuration store is configured with an AM installation"
    ...
    status=$?
    echo "[docker-entrypoint.sh] Is configured exit status is $status"
    return $status
}
...
bootstrap_openam() {
    ...
    if [ $? = 0 ];
    then
      echo "[docker-entrypoint.sh] Configstore is present, just run it!"
    else

      "${CATALINA_HOME}/bin/startup.sh"

      echo "[docker-entrypoint.sh] waiting restart of DS..."
      sleep 80
      ...
      echo "[docker-entrypoint.sh] we need to restart now"
...
run_amster_configurator(){
  ...
  case "$1" in
          "install")
            echo "[docker-entrypoint.sh] installation with local amster"
            ./amster amster_install.amster
            sed -r "s/^.*ssh-rsa/ssh-rsa/g" "${OPENAM_HOME}/"amster_rsa.pub >> "${OPENAM_HOME}/"authorized_keys
            ;;
          "import")
          echo "[docker-entrypoint.sh] import config"
            ./amster amster_import.amster
            ;;
          "update_cookie")
          echo "[docker-entrypoint.sh] update cookie name"
            ./amster amster_update_cookie_name.amster
            ;;
          *)
            echo "[docker-entrypoint.sh] if you read this message, there's something wrong...."
            ;;
  esac
}


echo "[docker-entrypoint.sh] Starting AM... "
bootstrap_openam
